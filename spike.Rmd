...
data_prc <- data %>%
  ...
  ungroup() %>%
  mutate(
    popularity_category = ifelse(popularity >= 80, "80+", "<80"),
...
song_words <- data_prc %>%
  select(year, name) %>%
  mutate(
    name_clean = tolower(str_replace_all(name, "['|']", "")),
...
test_wo_remaster <- test %>%
  filter(!word %in% c("remastered", "remaster", "remasterizado", "feat", "mix", "op", "remix", "ñœ", "ñ", "ð", "ðºð", "ã", "ðµ", "å", "ñƒð", "year", "years", "und", "con"))
```

```{r}
set.seed(100)
wordcloud(
  words = test_wo_remaster %>%
    head(100) %>% pull(word),
  freq = test %>%
...
test_2020 <- song_words %>%
  ...
  filter(n_songs > 1) %>%
  arrange(desc(n_songs)) %>%
  filter(!word %in% c("remastered", "remaster", "remasterizado", "feat", "mix", "mixed", "op", "remix", "ñœ", "ñ", "ð", "ðºð", "ã", "ðµ", "å", "ñƒð", "year", "years"))
```
